  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={() => { setEditId(null); resetForm(); setShowModal(true); }} className="cursor-pointer flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">
      ...
      <div className="mb-6 flex items-center gap-4 flex-wrap">
        ...
        />
        {selectedDate && (
          <button onClick={() => setSelectedDate("")} className="cursor-pointer text-xs text-slate-400 hover:text-white underline">Clear</button>
        )}
        <span className="text-xs text-slate-500">{selectedDate ? `Showing: ${selectedDate}` : "All dates"}</span>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <input
          ...
          className="w-full md:w-[40%] px-3 py-2 bg-slate-950 border border-slate-700 text-slate-100 rounded-lg text-sm focus:outline-none focus:border-blue-500 placeholder-slate-600 transition-colors"
        />
        <div className="flex gap-3">
          <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="cursor-pointer px-3 py-2 bg-slate-950 border border-slate-700 text-slate-300 rounded-lg text-sm focus:outline-none focus:border-blue-500">
            <option value="All">All Status</option>
            {ALL_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
          <select value={sortType} onChange={(e) => setSortType(e.target.value)} className="cursor-pointer px-3 py-2 bg-slate-950 border border-slate-700 text-slate-300 rounded-lg text-sm focus:outline-none focus:border-blue-500">
                  ...
                  <tr key={call.id} className="border-t border-slate-800 hover:bg-slate-800/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="flex justify-center items-center gap-3">
                        <button onClick={() => handleEdit(call)} className="cursor-pointer text-slate-400 hover:text-blue-400 transition-colors p-1">
                          <FaEdit />
                        </button>
