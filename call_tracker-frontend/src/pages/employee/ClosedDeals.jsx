...
export default function ClosedDeals() {
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        <input type="text" placeholder="Search closed deals..." value={search} onChange={(e) => setSearch(e.target.value)} className={INPUT} />
      </div>
              ...
              {filtered.map((call) => (
                <div key={call.id} className="p-4 hover:bg-slate-800/30 transition-colors">
                  <div className="flex items-start justify-between mb-1.5">
                    ...
                    <span className="ml-2 shrink-0 px-2 py-0.5 text-xs rounded-md bg-slate-500/10 text-slate-400">Closed</span>
                  </div>
                ...
                <tbody>
                  {filtered.map((call) => (
                    <tr key={call.id} className="border-t border-slate-800 hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3"><span className="px-2 py-0.5 text-xs rounded-md bg-slate-500/10 text-slate-400">Closed</span></td>
